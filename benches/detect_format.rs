...
fn detect_format(c: &mut Criterion) {
    ...
    g.bench_function("unflate", |b| {
        b.iter(|| black_box(niffler_temp::sniff(Box::new(BASIC_FILE))))
    });

    g.bench_function("gzip", |b| {
        b.iter(|| black_box(niffler_temp::sniff(Box::new(GZIP_FILE))))
    });
    g.bench_function("bzip", |b| {
        b.iter(|| black_box(niffler_temp::sniff(Box::new(BZIP_FILE))))
    });
    g.bench_function("lzma", |b| {
        b.iter(|| black_box(niffler_temp::sniff(Box::new(LZMA_FILE))))
    });
    g.bench_function("bgzip", |b| {
        b.iter(|| {
            black_box(niffler_temp::seek::sniff(Box::new(std::io::Cursor::new(
                BGZIP_FILE,
            ))))
        })
    });
